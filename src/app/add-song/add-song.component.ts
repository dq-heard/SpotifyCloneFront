...
import { ArtistVO, SaveSong, TitleVO } from '../service/model/song.model';
...
export class AddSongComponent implements OnDestroy {
  ...
  constructor() {
    effect(() => {
      ...
      if (this.songService.addSig().status === 'OK') {
        ...
        this.toastService.show('Error occured. Please try again.', 'DANGER');
      }
    });
  }
  ...
  create(): void {
    ...
    const artistVO: ArtistVO = { value: this.createForm.value.artist };

    this.songToCreate.title = titleVO;
    this.songToCreate.artist = artistVO;
